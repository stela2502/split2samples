...
impl CellData{
    ...
    pub fn n_umi<T: Index>( &self, gene_info:&T, gnames: &Vec<String> ) -> usize {
        let mut n = 0;
    ...
    pub fn n_reads<T: Index>( &self, gene_info:&T, gnames: &Vec<String> ) -> usize {
        //println!("I got {} umis for cell {}", n, self.name );
        let mut n = 0;
        for gname in gnames{
    ...
    pub fn n_umi_4_gene<T: Index>( &self, gene_info:&T, gname:&String) -> usize {
        let mut n = 0;
        let id = gene_info.get_id( gname.to_string() );
    ...
    pub fn to_str<T: Index>(&self, gene_info:&T, names: &Vec<String> ) -> String {

        let mut data = Vec::<std::string::String>::with_capacity( gene_info.names_len()+3 ); 
...
impl SingleCellData{
    ...
    pub fn write<T: Index> (&mut self, file_path: PathBuf, genes: &T, min_count:usize) -> Result< (), &str>{

        let mut names: Vec<String> = genes.names();
        return self.write_sub( file_path, genes, &names, min_count);
    }

    pub fn write_sub<T: Index> (&mut self, file_path: PathBuf, genes: &T, names: &Vec<String>, min_count:usize ) -> Result< (), &str>{
    ...
    pub fn write_sparse<T: Index> (&mut self, file_path: PathBuf, genes: &mut T, min_count:usize) -> Result< (), &str>{
        let names: Vec<String> = genes.names();

        return self.write_sparse_sub( file_path, genes, &names, min_count);
    }

    pub fn write_sparse_sub<T: Index> (&mut self, file_path: PathBuf, genes: &T, names: &Vec<String>, min_count:usize) -> Result< (), &str>{
        ...
        for name in genes.names4sparse() {
    ...
    pub fn update_names_4_sparse<T: Index>( &mut self, genes: &T, names:&Vec<String>) -> [usize; 2] {
    ...
    pub fn mtx_counts<T: Index>(&mut self, genes: &T, names: &Vec<String>, min_count:usize, num_threads: usize ) -> String{
        ...
        let ret = format!("{} {} {}", genes.max_id(), ncell_and_entries[0], ncell_and_entries[1] );
        //println!("mtx_counts -> final return: mtx_counts: {}", ret );
        ret
    }

    pub fn n_reads<T: Index>( &mut self, genes: &T, names: &Vec<String> ) -> usize {
        let mut count = 0;
