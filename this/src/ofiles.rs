...
impl Ofiles{
    pub fn new(id: usize, mode:&str, reads:&str, file:&str, outpath:&str )->Self {
        ...
        let fp1 = PathBuf::from(reads.clone());
        let fp2 = PathBuf::from(file.clone());
        ...
        let f1 = match File::create(file1_path){
            Ok(file) => file,
            Err(err) => panic!("The file {} cound not be created: {}", format!("{}.{}.{}", mode, id, fp1.file_name().unwrap().to_str().unwrap() ), err)
        };
        let f2 = match File::create(file2_path){
            Ok(file) => file,
            Err(err) => panic!("The file {} cound not be created: {}", format!("{}.{}.{}", mode, id, fp2.file_name().unwrap().to_str().unwrap() ), err)
        };
